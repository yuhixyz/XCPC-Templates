%!TEX program=xelatex
\documentclass[12pt,a4paper]{article}
%\usepackage{zh_CN-Adobefonts_external}
%\usepackage{xeCJK}
\usepackage{ctex}
\usepackage{amsmath, amsthm}
\usepackage{listings,xcolor}
\usepackage{geometry} % 设置页边距
\usepackage{fontspec}
\usepackage{graphicx}
\usepackage[colorlinks]{hyperref}
\usepackage{setspace}
\usepackage{fancyhdr} % 自定义页眉页脚
%\usepackage[toc]{multitoc} % 多栏目录，默认两栏

\hypersetup{linkcolor=black} % 设置目录为黑色

\setsansfont{Consolas} % 设置英文字体
\setmonofont[Mapping={}]{Courier New} % 英文引号之类的正常显示，相当于设置英文字体
\setlength{\parindent}{0pt} % 取消文章所有的首行缩进
\setlength{\headheight}{15pt} % 设置headheight

\linespread{1.2}

\title{Templates For ICPC}
\author{Yunhai Bian @ NJUPT}
\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

\pagestyle{fancy}

\lhead{Nanjing University of Posts and Telecommunications} %以下分别为左中右的页眉和页脚
\chead{}

\rhead{第 \thepage 页}
\lfoot{} 
\cfoot{\thepage}
\rfoot{}
\renewcommand{\headrulewidth}{0.4pt} 
\renewcommand{\footrulewidth}{0.4pt}
%\geometry{left=2.5cm,right=3cm,top=2.5cm,bottom=2.5cm} % 页边距
\geometry{left=2.5cm,right=2.3cm,top=2.5cm,bottom=2.5cm}
\setlength{\columnsep}{30pt}

\makeatletter

\makeatother


\lstset{
    language    = c++,
    numbers     = left,
    numberstyle={                               % 设置行号格式
        \small
        \color{black}
        \fontspec{Courier New}
    },
	commentstyle = \color{black}, %代码注释的颜色
	keywordstyle={                              % 设置关键字格式
        \color{black}
        \fontspec{Courier New Bold}
        \bfseries
    },
	stringstyle={                               % 设置字符串格式
        \color{black}
        \fontspec{Courier New}
    },
	basicstyle={                                % 设置代码格式
        \fontspec{Courier New}
        \small\ttfamily
    },
	emphstyle=\color[RGB]{112,64,160},          % 设置强调字格式
    breaklines=true,                            % 设置自动换行
    tabsize     = 4,
    frame       = single,%主题
    columns     = fullflexible,
    rulesepcolor = \color{red!20!green!20!blue!20}, %设置边框的颜色
    showstringspaces = false, %不显示代码字符串中间的空格标记
	escapeinside={\%*}{*)},
}

\begin{document}
\title{NJUPT XCPC Templates}
\author {Yunhai Bian}
\maketitle
\tableofcontents
\newpage
\section{数据结构}
\subsection{树状数组}
\lstinputlisting{数据结构/树状数组.cpp}
\subsection{线段树}
\subsubsection{HDU1540（维护lmax, rmax）}
\begin{spacing}{1.5}
\input{数据结构/线段树/HDU1540.tex}
\end{spacing}
\lstinputlisting{数据结构/线段树/HDU1540.cpp}
\subsubsection{HDU4578（维护多组懒标记）}
\begin{spacing}{1.5}
\input{数据结构/线段树/HDU4578.tex}
\end{spacing}
\lstinputlisting{数据结构/线段树/HDU4578.cpp}
\subsubsection{HDU4553（维护两棵有线段树）}
\begin{spacing}{1.5}
\input{数据结构/线段树/HDU4553.tex}
\end{spacing}
\lstinputlisting{数据结构/线段树/HDU4553.cpp}
\subsubsection{HDU1542（扫描线求矩形面积并）}
\begin{spacing}{1.5}
\input{数据结构/线段树/HDU1542.tex}
\end{spacing}
\lstinputlisting{数据结构/线段树/HDU1542.cpp}
\subsubsection{HDU1255（矩形2次覆盖面积并）}
\begin{spacing}{1.5}
\input{数据结构/线段树/HDU1255.tex}
\end{spacing}
\lstinputlisting{数据结构/线段树/HDU1255.cpp}
\subsection{分块}
\lstinputlisting{数据结构/分块.cpp}
\subsection{ST表}
\lstinputlisting{数据结构/ST表.cpp}
\subsection{Splay}
\lstinputlisting{数据结构/Splay.cpp}
\subsection{Treap}
\lstinputlisting{数据结构/Treap.cpp}
\subsection{树链剖分}
\subsubsection{点权}
\begin{spacing}{1.5}
\input{数据结构/树链剖分/点权.tex}
\end{spacing}
\lstinputlisting{数据结构/树链剖分/点权.cpp}
\subsubsection{边权}
\begin{spacing}{1.5}
\input{数据结构/树链剖分/边权.tex}
\end{spacing}
\lstinputlisting{数据结构/树链剖分/边权.cpp}
\subsection{莫队}
\subsubsection{离线询问排序方式}
\begin{spacing}{1.5}
\input{数据结构/莫队/离线询问排序方式.tex}
\end{spacing}
\subsubsection{洛谷P4396（基础莫队）}
\begin{spacing}{1.5}
\input{数据结构/莫队/洛谷P4396-[AHOI2013]作业（基础莫队）.tex}
\end{spacing}
\lstinputlisting{数据结构/莫队/洛谷P4396-[AHOI2013]作业（基础莫队）.cpp}
\subsubsection{AcWing2521（带修莫队）}
\begin{spacing}{1.5}
\input{数据结构/莫队/AcWing2521-数颜色（带修莫队）.tex}
\end{spacing}
\lstinputlisting{数据结构/莫队/AcWing2521-数颜色（带修莫队）.cpp}
\subsubsection{AcWing2523（回滚莫队）}
\begin{spacing}{1.5}
\input{数据结构/莫队/AcWing2523-历史研究（回滚莫队）.tex}
\end{spacing}
\lstinputlisting{数据结构/莫队/AcWing2523-历史研究（回滚莫队）.cpp}
\subsubsection{树上莫队}
\begin{spacing}{1.5}
\input{数据结构/莫队/树上莫队.tex}
\end{spacing}
\section{图论}
\subsection{最短路}
\subsubsection{朴素Dijkstra}
\lstinputlisting{图论/最短路/朴素Dijkstra.cpp}
\subsubsection{堆优化Dijkstra}
\lstinputlisting{图论/最短路/堆优化Dijkstra.cpp}
\subsubsection{Bellman-Ford}
\lstinputlisting{图论/最短路/Bellman-Ford.cpp}
\subsubsection{SPFA}
\lstinputlisting{图论/最短路/SPFA.cpp}
\subsubsection{Floyd}
\lstinputlisting{图论/最短路/Floyd.cpp}
\subsection{最小生成树}
\subsubsection{Prim}
\lstinputlisting{图论/最小生成树/Prim.cpp}
\subsubsection{Kruskal}
\lstinputlisting{图论/最小生成树/Kruskal.cpp}
\subsection{严格次小生成树}
\lstinputlisting{图论/严格次小生成树.cpp}
\subsection{有向图的强连通分量}
\lstinputlisting{图论/有向图的强连通分量.cpp}
\subsection{无向图的双连通分量}
\subsubsection{AcWing1189（点的双连通分量）}
\lstinputlisting{图论/无向图的双连通分量/AcWing1189.cpp}
\subsubsection{AcWing396（点的双连通分量、割点）}
\lstinputlisting{图论/无向图的双连通分量/AcWing396.cpp}
\subsubsection{AcWing395（边的双连通分量、桥）}
\lstinputlisting{图论/无向图的双连通分量/AcWing395.cpp}
\subsection{差分约束}
\subsubsection{AcWing362（任意边权）}
\begin{spacing}{1.5}
\input{图论/差分约束/任意边权.tex}
\end{spacing}
\lstinputlisting{图论/差分约束/任意边权.cpp}
\subsubsection{AcWing368（01边权）}
\begin{spacing}{1.5}
\input{图论/差分约束/01边权.tex}
\end{spacing}
\lstinputlisting{图论/差分约束/01边权.cpp}
\subsection{最近公共祖先}
\subsubsection{倍增}
\lstinputlisting{图论/最近公共祖先/倍增.cpp}
\subsubsection{Tarjan}
\lstinputlisting{图论/最近公共祖先/Tarjan.cpp}
\subsubsection{树剖}
\lstinputlisting{图论/最近公共祖先/树剖.cpp}
\subsection{网络流}
\subsubsection{Dinic}
\begin{spacing}{1.5}
\input{图论/网络流/Dinic.tex}
\end{spacing}
\lstinputlisting{图论/网络流/Dinic.cpp}
\subsubsection{EK}
\lstinputlisting{图论/网络流/EK.cpp}
\subsection{二分图}
\subsubsection{染色法}
\lstinputlisting{图论/二分图/染色法.cpp}
\subsubsection{匈牙利算法}
\lstinputlisting{图论/二分图/匈牙利算法.cpp}
\subsubsection{最大流之二分图最大匹配}
\lstinputlisting{图论/二分图/最大流之二分图最大匹配.cpp}
\subsubsection{最大流之二分图多重匹配}
\lstinputlisting{图论/二分图/最大流之二分图多重匹配.cpp}
\section{字符串}
\subsection{Manacher}
\lstinputlisting{字符串/Manacher.cpp}
\subsection{KMP}
\lstinputlisting{字符串/KMP.cpp}
\subsection{AC自动机}
\lstinputlisting{字符串/AC自动机.cpp}
\section{其他}
\subsection{离散化}
\lstinputlisting{其他/离散化.cpp}
\subsection{高精度}
\lstinputlisting{其他/高精度.cpp}
\end{document}